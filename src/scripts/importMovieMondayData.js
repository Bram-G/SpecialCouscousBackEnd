...
async function getOrCreateUser(username, email, password = 'Password') {
  try {
...
async function fetchMovieDetails(tmdbId) {
  ...
  try {
    ...
    if (!response.ok) {
      console.warn(
        `Failed to fetch movie details for ID ${tmdbId}: ${response.status}`,
      );
      return null;
    }
...
async function processCSVData(csvData) {
  ...
  if (!group) {
    ...
    group = await Group.create({
      ...
      slug: "movie-monday",
      description:
        "Our weekly tradition of watching movies together. Every Monday we pick new films, enjoy themed drinks and food, and create lasting memories.",
    });

    // Add all users to the group
    await Promise.all(
      Object.values(users).map((user) => group.addUser(user.id)),
    );
  ...
  for (const [index, row] of parsedData.entries()) {
    try {
      ...
      console.log(
        `Processing MovieMonday for ${formattedDate}, picker: ${pickerName}`,
      );

      // Randomly assign a theme to some weeks (about 30% of them)
      const shouldHaveTheme = Math.random() < 0.3;
      const weekTheme = shouldHaveTheme
        ? weekThemes[Math.floor(Math.random() * weekThemes.length)]
        : null;
      ...
      const movieIds = [
        {
          id: row["Winning TMDB ID"],
          title: row["Winning Movie"],
          isWinner: true,
        },
          ...
          if (movieDetails.credits) {
            ...
            for (const actor of topCast) {
              try {
                ...
                console.error(
                  `Error adding cast member ${actor.name}:`,
                  castError.message,
                );
              }
            }
            ...
            for (const person of uniqueCrew) {
              try {
                ...
                console.error(
                  `Error adding crew member ${person.name}:`,
                  crewError.message,
                );
              }
            }
            ...
            console.log(
              `  Added ${topCast.length} cast and ${uniqueCrew.length} crew members to movie "${movieDetails.title}"`,
            );
          }
        }
      }
      ...
      const cocktails =
        row.Cocktail && row.Cocktail.trim() !== "" && row.Cocktail !== "[ ]"
          ? row.Cocktail.split(",")
              .map((c) => c.trim())
              .filter(Boolean)
          : [];
      ...
      const meals =
        row.Dinner && row.Dinner.trim() !== "" && row.Dinner !== "[ ]"
          ? [row.Dinner.trim()]
          : [];

      const desserts =
        row.Dessert && row.Dessert.trim() !== "" && row.Dessert !== "[ ]"
          ? [row.Dessert.trim()]
          : [];
      ...
      try {
        ...
        const hasData =
          cocktails.length > 0 || meals.length > 0 || desserts.length > 0;

        if (hasData) {
          // Create or update event details
          const [eventDetails, eventDetailsCreated] =
          ...
          console.log(
            `  Event details ${eventDetailsCreated ? "created" : "updated"}`,
          );
        }
      } catch (eventError) {
...
async function importMovieMondayData() {
  try {
    ...
    // Recalculate stats once, instead of via hundreds of per-row hook calls
    console.log("Recalculating statistics...");
    const { Statistic } = require("../models");
    await Statistic.recalculateAll(require("../models"));
