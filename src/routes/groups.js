...
router.post('/groups/:groupId/leave', auth, async (req, res) => {
  try {
    ...
    if (group.createdById === userId) {
      return res.status(400).json({ error: 'Group owner cannot leave the group' });
    }
...
router.post('/groups/:groupId/invite-link', auth, async (req, res) => {
  try {
    ...
    if (!group) {
      return res.status(403).json({ error: 'Not authorized to create invite link for this group' });
    }
...
router.post('/groups/:groupId/invites', auth, async (req, res) => {
  try {
    ...
    const invitedUser = await User.findOne({ where: { email: invitedUserEmail } });
    if (!invitedUser) {
