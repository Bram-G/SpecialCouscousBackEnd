...
const validateContentAccess = async (contentType, contentId, userId = null) => {
  switch (contentType) {
    ...
    case "moviemonday":
      const movieMonday = await MovieMonday.findByPk(contentId, {
        include: [
          {
            model: Group,
      ...
      if (!movieMonday) {
        return { valid: false, error: "Movie Monday not found" };
      }

      // Check access: public entries are accessible to all, private only to group members
      const isPublicAccess = movieMonday.isPublic && movieMonday.Group.isPublic;
      const isMemberAccess =
        userId && (await checkGroupMembership(userId, movieMonday.GroupId));

      if (!isPublicAccess && !isMemberAccess) {
        return {
          valid: false,
          error: "Not authorized to view this Movie Monday",
        };
      }

      return {
        valid: true,
        title: `Movie Monday - ${movieMonday.date}`,
        isOwner: userId === movieMonday.pickerUserId,
        movieMonday,
      };
...
const getCommentsForContent = async (contentType, contentId, options = {}) => {
  ...
  const validation = await validateContentAccess(
    contentType,
    contentId,
    userId
  );
  ...
  async (req, res) => {
    try {
      ...
      if (!contentId || isNaN(contentId)) {
        return res
          .status(400)
          .json({ message: "Valid content ID is required" });
      }
