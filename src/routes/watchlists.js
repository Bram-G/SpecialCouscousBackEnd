...
router.delete("/categories/:id", auth, async (req, res) => {
  ...
  try {
    ...
    if (categoriesCount === 1) {
      return res
        .status(400)
        .json({ message: "Cannot delete the only watchlist category" });
    }
...
router.post("/categories/:id/movies", auth, async (req, res) => {
  ...
  try {
    ...
    const maxSortOrder =
      (await WatchlistItem.max("sortOrder", {
        where: { categoryId: id },
      })) || 0;
...
router.delete(
  "/categories/:categoryId/movies/:itemId",
  auth,
  async (req, res) => {
    ...
    try {
      ...
      if (!watchlistItem) {
        return res
          .status(404)
          .json({ message: "Watchlist item not found or not authorized" });
      }
...
router.post("/categories/:id/like", auth, async (req, res) => {
  ...
  try {
    ...
    if (!category) {
      return res
        .status(404)
        .json({ message: "Public watchlist category not found" });
    }
...
router.post("/add-to-watchlists", auth, async (req, res) => {
  ...
  if (!tmdbMovieId || !title || !categoryIds || !Array.isArray(categoryIds)) {
    return res
      .status(400)
      .json({ message: "Movie details and category IDs array are required" });
  }
  ...
  try {
    ...
    if (categories.length !== categoryIds.length) {
      ...
      return res
        ...
        .json({
          message:
            "One or more watchlist categories not found or not authorized",
        });
    }
...
router.post("/copy-movie", auth, async (req, res) => {
  ...
  if (!sourceItemId || !targetCategoryId) {
    return res
      .status(400)
      .json({ message: "Source item ID and target category ID are required" });
  }
  ...
  try {
    ...
    if (!sourceItem) {
      return res
        .status(404)
        .json({ message: "Source watchlist item not found or not authorized" });
    }
    ...
    if (!targetCategory) {
      return res
        .status(404)
        .json({
          message: "Target watchlist category not found or not authorized",
        });
    }
    ...
    res
      .status(500)
      .json({ message: "Failed to copy movie between watchlists" });
  }
});
...
router.get("/status/:tmdbMovieId", auth, async (req, res) => {
  try {
    ...
    const inDefaultWatchlist = watchlistItems.some(
      (item) => item.WatchlistCategory.id === defaultWatchlist.id
    );
...
router.post("/quick-add", auth, async (req, res) => {
  try {
    ...
    const result = await addToDefaultWatchlist(req.user.id, {
      tmdbMovieId,
      title,
      posterPath,
    });
